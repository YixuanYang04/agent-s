...
This repository includes local Windows launchers for Agent-S3:

- `test_windows.bat` tests the main model and the remote UI-TARS grounding model.
- `run_cli.bat` creates a local `.venv` on first run, opens the SSH tunnel to the remote vLLM server, and starts the CLI.
- `start_remote_model_tunnel.bat` only opens/checks the SSH tunnel.

See [WINDOWS_ONE_CLICK.md](WINDOWS_ONE_CLICK.md) for the full Windows setup notes.
