...
log_dir = os.getenv("AGENT_S_LOG_DIR", "logs")
...
def run_agent(
    ...
    """Runs until agent outputs done/fail, or hits max_steps safety cap (never 15-forced)."""
    global paused
    obs = {}
    traj = "Task:\n" + instruction
    subtask_traj = ""
    if max_steps is None:
        raw = os.environ.get("AGENT_GUI_MAX_ITERATIONS", "8000").strip()
        max_steps = int(raw) if raw.isdigit() and int(raw) > 0 else 8000
    ...
    while True:
        ...
        if step > max_steps:
            print(
                f"\n⚠️ [Agent] 已达安全上限 {max_steps} 步（未完成 done/fail）。"
                f" 可提高环境变量 AGENT_GUI_MAX_ITERATIONS 或参数 --max-agent-steps。"
            )
            break
        ...
        print(f"\n🔄 Step {step} (≤{max_steps} 安全上限，正常以输出 done/fail 结束)...")
...
def main():
    ...
    parser.add_argument(
        "--task",
        type=str,
        help="Task for Agent-S3; if set with --task-file or non-empty, runs once and exits (no Query loop).",
    )
    parser.add_argument(
        "--task-file",
        type=str,
        default="",
        help="UTF-8 file with task text; if file exists, overrides --task.",
    )
    parser.add_argument(
        "--max-agent-steps",
        type=int,
        default=None,
        metavar="N",
        help="Hard safety cap on GUI loop iterations. Omitted: use env AGENT_GUI_MAX_ITERATIONS (default 3000). Normal exit: agent outputs done/fail.",
    )
    ...
    task = ""
    if getattr(args, "task_file", None) and str(args.task_file).strip():
        tfp = Path(str(args.task_file).strip()).expanduser()
        if tfp.is_file():
            task = tfp.read_text(encoding="utf-8")
    if not (task and task.strip()) and args.task and str(args.task).strip():
    ...
    while True:
        ...
        run_agent(agent, query, scaled_width, scaled_height, max_steps=args.max_agent_steps)
